...
class cleanData:
    ...
    def handleDropNulValue(self, band):
        subset_cols = band if isinstance(band, list) else [band]
        # chỉ giữ column tồn tại
        subset_cols = [col for col in subset_cols if col in self.inputDf.columns]

        if len(subset_cols) == 0:
            return "No valid columns to drop NA"

        self.inputDf = self.inputDf.dropna(subset=subset_cols).reset_index(drop=True)
        return f"length of removed null value dataframe: {len(self.inputDf)}"
...
if __name__ == "__main__":
    ...
    nullCleanBand = [
        'B09B','B10B','B11B','B12B','B14B','B16B','I2B','I4B','IRB','WVB','NDVI'
    ]
    # Choose time bands (*)
    startPeriod = -6
    endPeriod = 6
    ...
    for filepath in tqdm(fullFile, total= len(fullFile)):
        ...
        cleanObject.handleDropNulValue(bandNullType)
        logger.info("Handle null value completed")
        ...
        # *
        outputFile = f"/sdd/Dubaoset/src/Phong/Model/data/validation/validationCleaned/{file_name}"
