...
#     def __init__(self, alpha=0.25, gamma=2.0, weight_reg=1.0, weight_cls=1.0):
#         super().__init__()
#         self.alpha = alpha
#         self.gamma = gamma
...
#         # Regression loss (MSE)
#         y_pred_reg = y_pred_reg.squeeze(-1)  # (B,1) → (B,)
#         y_pred_cls = y_pred_cls.squeeze(-1)  # (B,1) → (B,)

#         mse_loss = nn.functional.mse_loss(y_pred_reg, y_reg.float(), reduction='mean')
        
#         # Classification loss (Focal Loss)
#         bce = nn.functional.binary_cross_entropy_with_logits(
#             y_pred_cls.squeeze(), y_cls.float().squeeze(), reduction='none'
#         )
#         pt = torch.exp(-bce)
#         alpha_t = self.alpha * y_cls.float().squeeze() + (1 - self.alpha) * (1 - y_cls.float().squeeze())
#         focal_loss = alpha_t * ((1 - pt) ** self.gamma) * bce
        
#         total_loss = self.weight_reg * mse_loss + self.weight_cls * focal_loss.mean()
