...
VAL_PATH = '/sdd/Dubaoset/src/Thang/DataMB/TrainProcess/test_data.parquet'
# TEST_DATA_DIR = "/sdd/Dubaoset/src/Phong/Model/data/testClean"

test_5_path_dir = "/sdd/Dubaoset/src/Thang/DataMB/Test/5"
test_6_path_dir = "/sdd/Dubaoset/src/Thang/DataMB/Test/6"
test_7_path_dir = "/sdd/Dubaoset/src/Thang/DataMB/Test/7"
list_File_Test = [os.path.join(test_6_path_dir, f) for f in os.listdir(test_6_path_dir)] + [os.path.join(test_7_path_dir, f) for f in os.listdir(test_7_path_dir)]
file_pos_test = "/sdd/Dubaoset/src/Thang/DataMB/Test/pos/test/batch_0001_from_00000_to_00000.parquet"

list_File_Test += [file_pos_test]



listFileOk = list_File_Test
listFileNotOk = [os.path.join(test_5_path_dir, f) for f in os.listdir(test_5_path_dir)]
# listFile = [os.path.join(TEST_DATA_DIR, f) for f in os.listdir(TEST_DATA_DIR) if f.endswith('.parquet')]
print(len(listFileOk))
print(len(listFileNotOk))

file_list_str = ", ".join([f"'{f}'" for f in listFileOk])
file_list_str_not_ok = ", ".join([f"'{f}'" for f in listFileNotOk])

bandName = ['B09B','B10B','B11B','B12B','B14B','B16B','I2B','I4B','IRB','WVB', 'NDVI','Dem_value']

# File xuất Cache
cache_json_output = '/sdd/Dubaoset/src/Thang/DataMB/TrainProcess/plot/total_test_data_master_cache.json'
...
for band in bandName:
    ...
    for i in range(6):
        ...
        for t_val in range(window_start, window_end):
            ...
            all_window_queries.append(
                f'SELECT "{col_name}" AS val, "{label_col}" AS label FROM read_parquet([{file_list_str_not_ok}]) WHERE output_0 = 0'  # Chỉ lấy class 0 từ tập không ok
            )
