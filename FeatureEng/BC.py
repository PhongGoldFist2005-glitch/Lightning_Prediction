...
if __name__ == "__main__":
    ...
    for i in range(startPos, endPos, stepPos):
        ...
        result = eval_point_biserial(X=X, y= y, timeStamps= timeStamps, bandName= fullBand)
        topBand = list(result.items())

        logger.info(
            f"[PointBiserial] Top bands: "
            f"{[(k, round(v['corr'], 4)) for k, v in topBand]}"
        )
